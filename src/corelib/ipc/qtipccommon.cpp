...
static QNativeIpcKey::Type stringToType(QStringView typeString)
{
    ...
    if (typeString == staticTypeToString(QNativeIpcKey::Type::OS2))
        return QNativeIpcKey::Type::OS2;
                                                 ...
                                                 QNativeIpcKey::Type type)
{
    ...
    case QNativeIpcKey::Type::OS2:
        ...
        int maxlen = 255 - native.length();
        if (result.length() > maxlen)
            result = result.right(maxlen);
        QNativeIpcKeyPrivate::setNativeAndLegacyKeys(k, native + result, key);
        return k;
        }
    case QNativeIpcKey::Type::SystemV:
        break;
                                           ...
                                           QNativeIpcKey::Type type)
{
    ...
    case QNativeIpcKey::Type::OS2:
// FIXME
//            k.setNativeKey(QStringView(key).left(PATH_MAX - 1));
//        return k;
        break;
